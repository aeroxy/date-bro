...
describe('mergeResearchNotes', () => {
  ...
  test('skips a line already present, however it was bulleted', () => {
    expect(mergeResearchNotes('- One', ['* one', 'one', 'two'])).toBe('- One\n- two')
  })
...
describe('replaceResearchNotes', () => {
  const snapshot = '- the studio closes at nine\n- studio shuts 9pm\n- one week of annual leave left'

  test('swaps the whole block for the consolidated list', () => {
    const consolidated = ['the studio closes 9pm', 'one week of annual leave left']
    expect(replaceResearchNotes(snapshot, snapshot, consolidated)).toBe(
  ...
  test('keeps a line the user typed while the run was in flight', () => {
    ...
    expect(replaceResearchNotes(snapshot, current, ['the studio closes 9pm'])).toBe(
      '- the studio closes 9pm\n- sings in a choir',
    )
  })
  ...
  test('does not re-add a user line the consolidation already covered', () => {
    const current = `${snapshot}\n- sings in a choir`
    expect(
      replaceResearchNotes(snapshot, current, ['the studio closes 9pm', 'Sings in a choir']),
    ).toBe('- the studio closes 9pm\n- Sings in a choir')
  })

  // The other direction of the same race, and the one a swap gets wrong by
  // default: the model was given this line and hands it back, having been told
  // to return everything still true. It isn't still true — the user deleted it
  // while the run was in flight, and the model had no way to know.
  test('does not resurrect a line the user deleted while the run was in flight', () => {
    const current = '- the studio closes at nine\n- studio shuts 9pm'
    expect(
  ...
  // `ProfileModal` has a Clear button, so the whole-block version of that is one
  // click. Anything genuinely new in the consolidation still lands — it isn't
  // something the user decided against.
  test('honours a Clear, keeping only what the model added', () => {
    expect(replaceResearchNotes(snapshot, '', ['studio shuts 9pm', 'they row on Sundays'])).toBe(
      '- they row on Sundays',
    )
  })
  ...
  // guard reads the returned list rather than the reconciled one.
...
describe('applyResearchNotes', () => {
  test('appends below the ceiling', () => {
    const notes = lines(3)
    expect(applyResearchNotes(notes, notes, ['fact 9'])).toBe(`${notes}\n- fact 9`)
  })

  test('replaces above it', () => {
    const notes = lines(NOTES_LINE_CEILING + 1)
    expect(applyResearchNotes(notes, notes, ['fact 0', 'fact 1'])).toBe('- fact 0\n- fact 1')
  })
  ...
  test('follows the snapshot, not the notes as they stand now', () => {
    ...
    expect(applyResearchNotes(snapshot, '- fact 0', ['merged'])).toBe('- merged')
  })
})
